...
public class Response {
    public static void main (String[] args) {
        ...
        socket.bind ("tcp://localhost:5555");    //绑定端口
        int i = 0;
        int number = 0;
        while (!Thread.currentThread().isInterrupted()) {
            ...
            byte[] request = socket.recv(0);  //获取request发送过来的数据
            //System.out.println("receive : " + new String(request));
            String response = "world";
            socket.send(response.getBytes(),1);  //向request端发送数据  ，必须要要request端返回数据，没有返回就又recv，将会出错，这里可以理解为强制要求走完整个request/response流程
        }
