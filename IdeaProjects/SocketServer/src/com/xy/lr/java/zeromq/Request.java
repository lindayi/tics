            ...
            new Thread(new Runnable(){
                public void run() {
                    ...
                    for (int i = 0; i < 100000; i++) {
                        ...
                        socket.send(request.getBytes(),1);   //向reponse端发送数据
                        byte[] response = socket.recv(0);   //接收response发送回来的数据  正在request/response模型中，send之后必须要recv之后才能继续send，这可能是为了保证整个request/response的流程走完
